package in.koreatech.koin.domain.session.service;
...
import in.koreatech.koin.domain.session.repository.UserSessionRedisRepository;
...
public class UserSessionService {
    ...
    private UserSession updateUserStatus(Integer userId, Long articleId, Long chatRoomId, UserSessionStatus status) {
        UserSession userSession = userSessionRepository.findUserSession(userId)
            .orElseThrow(() -> new UserSocketSessionException(UserSocketSessionErrorCode.REDIS_SESSION_FIND_FAILED));
